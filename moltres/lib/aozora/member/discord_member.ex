defmodule Aozora.Member.DiscordMember do
  ...
  def changeset(discord_member, attrs) do
    discord_member
    |> cast(attrs, [:discord_id, :discord_username, :discord_avatar, :discord_banner, :discord_accent_color, :discord_mfa_enabled, :discord_verified, :last_study_date, :study_streak, :total_days_studied])
    |> validate_required([:discord_id, :discord_username, :discord_discriminator])
    |> validate_length(:discord_discriminator, min: 4)
    |> validate_length(:discord_discriminator, max: 4)
